...
int lf_patterns_build(lf_patterns_t *ps, const grep_opts_t *o) {
    ...
    lf_search_kind_t kind = o->extended_regexp ? LF_SEARCH_REGEX : LF_SEARCH_FIXED;
    ps->searchers = (lf_searcher_t *)lf_xcalloc(o->n_patterns, sizeof(lf_searcher_t));
    for (size_t i = 0; i < o->n_patterns; i++) {
        if (lf_search_init(&ps->searchers[i], o->patterns[i], o->ignore_case, kind) != 0) {
            fprintf(stderr, "ambil: invalid %s pattern '%s'\n",
                    o->extended_regexp ? "regex" : "fixed", o->patterns[i]);
            return -1;
        }
        ps->n++;
    }
...
static void json_emit_begin(emit_ctx_t *e) {
    ...
    lf_json_escape(e->out, e->path, strlen(e->path));
    lf_buf_append(e->out, "\"}\n", 3);
}
static void json_emit_event(emit_ctx_t *e, const char *type, uint64_t lineno,
                            ...
                            const lf_match_span_t *spans, size_t n_spans,
                            const char *kind) {
    lf_buf_appendf(e->out, "{\"type\":\"%s\",\"path\":\"", type);
    lf_json_escape(e->out, e->path, strlen(e->path));
    lf_buf_appendf(e->out, "\",\"line_number\":%llu,\"lines\":\"", (unsigned long long)lineno);
    lf_json_escape(e->out, line, llen);
    lf_buf_append(e->out, "\"", 1);
    if (kind) lf_buf_appendf(e->out, ",\"kind\":\"%s\"", kind);
    if (spans && n_spans > 0) {
        lf_buf_append(e->out, ",\"submatches\":[", 15);
        for (size_t i = 0; i < n_spans; i++) {
            if (i) lf_buf_append(e->out, ",", 1);
            lf_buf_appendf(e->out, "{\"start\":%zu,\"end\":%zu,\"text\":\"", spans[i].start, spans[i].end);
            lf_json_escape(e->out, line + spans[i].start, spans[i].end - spans[i].start);
...
static void json_emit_end(emit_ctx_t *e, const lf_file_stats_t *st) {
    lf_buf_appendf(e->out,
        "{\"type\":\"end\",\"path\":\"");
    lf_json_escape(e->out, e->path, strlen(e->path));
    lf_buf_appendf(e->out, "\",\"stats\":{\"matched_lines\":%llu,\"searches\":%llu}}\n",
...
/* Internal scan. full_data/full_size cover the whole file (for binary
 * detection); data/size cover the slice this call should scan. For non-
 * chunked calls full == data. base_lineno is the line number BEFORE the
 * first line in `data` (chunk's offset translated to line count).
 * suppress_heading prevents per-call heading emission for chunk_idx>0. */
static int lf_grep_buffer_impl(const char *data, size_t size,
                                const char *full_data, size_t full_size,
                                const char *rel_path,
    ...
    if (size == 0) return 0;
    if (!opt->text_mode && lf_is_binary(full_data, full_size)) return 0;
    ...
    E.out = out;
    if (suppress_heading) E.header_emitted = 1;
    ...
    int header_pending_json = mode_json && !suppress_heading;
    ...
    /* Iterate lines. Line numbering starts at base_lineno + 1 so chunk
     * outputs show correct global line numbers. */
    uint64_t lineno = base_lineno;
    size_t i = 0;
    while (i < size) {
    ...
    /* Mode-final emissions. Skip per-chunk JSON end / blank-line trailers when
     * we're a continuation chunk — only the first/whole call emits them. */
    if (mode_c) {
        if (stats->matched_lines > 0 && !suppress_heading) {
            lf_buf_appendf(out, "%s:%llu\n", rel_path, (unsigned long long)stats->matched_lines);
        }
    } else if (mode_json && stats->had_match && !suppress_heading) {
        json_emit_end(&E, stats);
    } else if (E.header_emitted && opt->mode != OUT_COMPACT && !suppress_heading) {
...
int lf_grep_buffer(const char *data, size_t size,
                   ...
                   const grep_opts_t *opt, const lf_patterns_t *ps,
                   int color, lf_buf_t *out, lf_file_stats_t *stats) {
    return lf_grep_buffer_impl(data, size, data, size, rel_path,
                                opt, ps, color, out, stats, 0, 0);
}
...
int lf_grep_buffer_chunk(const char *full_data, size_t full_size,
                         ...
                         int color, lf_buf_t *out, lf_file_stats_t *stats,
                         uint64_t base_lineno, int suppress_heading) {
    return lf_grep_buffer_impl(full_data + chunk_off, chunk_len,
                                full_data, full_size,
                                rel_path, opt, ps, color, out, stats,
                                base_lineno, suppress_heading);
}
...
int lf_grep_path(const char *path, const char *rel_path,
    ...
    /* Pre-size output buffer to reduce reallocs on grep-heavy files.
     * Skip in stream mode — buffer is bypassed. */
    if (!out->stream) {
        double ratio = (opt->mode == OUT_JSON) ? 0.45 : 0.30;
        size_t presize = (size_t)((double)m.size * ratio) + 256;
        if (presize > 16 * 1024 * 1024) presize = 16 * 1024 * 1024;
        lf_buf_reserve(out, presize);
    }

    int rc = lf_grep_buffer(m.data, m.size, rel_path, opt, ps, color, out, stats);
    lf_mmap_close(&m);
    return rc;
}
