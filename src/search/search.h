...
typedef enum {
    LF_SEARCH_FIXED = 0,   /* Boyer-Moore-Horspool / SIMD substring */
    LF_SEARCH_REGEX = 1    /* tiny embedded regex (re_tiny) */
} lf_search_kind_t;

typedef struct {
    int                  kind;       /* lf_search_kind_t */
    const unsigned char *pat;        /* lowercased copy if ignore_case else original (fixed mode) */
    size_t               plen;
    int                  ignore_case;
    size_t               skip[256];
    unsigned char       *owned;      /* lowercased buffer, freed by lf_search_free */
    lf_re_program_t      re;         /* compiled regex (kind == LF_SEARCH_REGEX) */
} lf_searcher_t;
 ...
 *
 * kind selects fixed-string (BMH/SIMD) vs regex (re_tiny — `. * + ? [] ^ $
 * \d \w \s` and inversions; no groups/alternation/backrefs).
 */
